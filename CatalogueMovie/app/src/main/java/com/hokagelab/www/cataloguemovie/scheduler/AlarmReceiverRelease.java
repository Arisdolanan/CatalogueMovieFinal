...
public class AlarmReceiverRelease extends BroadcastReceiver {
    ...
    private void getMovieDays(final Context context, int notifId){
        ...
        call.enqueue(new Callback<MovieResponse>() {
            ...
            public void onResponse(Call<MovieResponse> call, Response<MovieResponse> response) {
                ...
                String title = MovieList.get(MovieList.size() - 1).getTitle();
                String message = MovieList.get(MovieList.size() - 1).getOverview();
                String poster = MovieList.get(MovieList.size() - 1).getPoster_path();

                int notifId = 200;

                    showAlarmNotification(context,title, message, notifId+2, poster);
    ...
    public void setRepeat(Context context, String type, String time, String message){
        ...
        calendar.set(Calendar.AM_PM, 0);  //am 0 //pm 1
        int requestCode = NOTIF_ID_REPEATING;
        PendingIntent pendingIntent =  PendingIntent.getBroadcast(context, requestCode, intent, 0);

        alarmManager.setInexactRepeating(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), AlarmManager.INTERVAL_DAY, pendingIntent);
        Toast.makeText(context, "Notification Daily On", Toast.LENGTH_SHORT).show();
